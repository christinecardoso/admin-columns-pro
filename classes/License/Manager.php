...
class Manager
	...
	public function inject_addon_install_resource( $result, $action, $args ) {
		...
		$data = $this->get_cached_install_data( $args->slug );
	...
	public function maybe_install_addon_error( $error, $plugin_name ) {
		...
		$data = $this->get_cached_install_data( $plugin_name );
	 ...
	 * @throws \Exception
	 */
	private function get_cached_install_data( $plugin_name ) {
		$cache = new Transient( 'acpplugin_install_' . $plugin_name );

		if ( $cache->is_expired() ) {
			$install = new Install( $plugin_name, $this->api );

			$result = $install->get_install_data();
	...
	private function get_plugins() {
		...
		foreach ( $deprecated_addons as $dirname ) {
			$plugin = PluginInformation::create_by_dirname( $dirname );
